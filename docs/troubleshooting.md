...
Policy failures list the denied name/import or a safe limit. Imports are limited to `bpy`, `bmesh`, `mathutils`, `math`, `json`, `collections`, `functools`, `itertools`, `random`, and `statistics`; shell, process, network, filesystem, dynamic-code, and double-underscore introspection paths are rejected. The policy is an accident guard, not a hard security sandbox.
